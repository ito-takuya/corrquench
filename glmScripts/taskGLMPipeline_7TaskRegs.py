...
basedir = '/projects3/TaskFCMech/data/'
datadir = basedir + 'hcppreprocessedmsmall/'
...
def runGroupTaskGLM(nuisModel='24pXaCompCorXVolterra', taskModel='FIR', nproc=10, byRun=False):
    scount = 1
    for subj in subjNums:
        print 'Running task GLM for subject', subj, '|', scount, '/', len(subjNums), '... model:', taskModel
        if byRun:
            for task in taskNames:
                taskGLM_byRun(subj, task, nuisModel=nuisModel, nproc=nproc)
        else:
            for task in taskNames:
...
def taskGLM(subj, task, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8):
    ...
    h5f = h5py.File(outputdir + subj + '_glmOutput_data.h5','a')
    outname1 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_7TaskRegs_resid_' + taskModel
    outname2 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_7TaskRegs_betas_' + taskModel
    try:
...
def taskGLM_byRun(subj, task, nuisModel='24pXaCompCorXVolterra', nproc=8):
    """
    This function runs a task-based GLM on a single subject
    Will only regress out task-timing, using either a canonical HRF model or FIR model
    ...
    h5f = h5py.File(outputdir + subj + '_glmOutput_data.h5','a')
    outname1 = 'taskRegression/' + task + '_RL_' + nuisModel + '_taskReg_7TaskRegs_resid_canonical'
    outname2 = 'taskRegression/' + task + '_RL_' + nuisModel + '_taskReg_7TaskRegs_betas_canonical' 
    try:
        h5f.create_dataset(outname1,data=residual_ts)
        h5f.create_dataset(outname2,data=betas)
    except:
    ...
    h5f = h5py.File(outputdir + subj + '_glmOutput_data.h5','a')
    outname1 = 'taskRegression/' + task + '_LR_' + nuisModel + '_taskReg_7TaskRegs_resid_canonical'
    outname2 = 'taskRegression/' + task + '_LR_' + nuisModel + '_taskReg_7TaskRegs_betas_canonical' 
    try:
        h5f.create_dataset(outname1,data=residual_ts)
        h5f.create_dataset(outname2,data=betas)
    except:
...
def loadTaskTiming(subj, task, taskModel='canonical', nRegsFIR=25):
    ...
    ### Single task regressor
    stimMat = stimMat = np.sum(stimMat,1)
    stimMat.shape = (len(stimMat),1)
