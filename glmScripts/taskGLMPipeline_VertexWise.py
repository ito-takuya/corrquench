...
outputdir = '/projects2/HCP352Data/data/hcppreprocessedmsmall/vertexWise/'
# TRlength
trLength = .720
...
def taskGLM_byRun(subj, task, nuisModel='24pXaCompCorXVolterra', nproc=8):
    ...
    h5f = h5py.File(outputdir + subj + '_glmOutput_64k_data.h5','a')
    outname1 = 'taskRegression/' + task + '_LR_' + nuisModel + '_taskReg_resid_canonical'
    outname2 = 'taskRegression/' + task + '_LR_' + nuisModel + '_taskReg_betas_canonical' 
    try:
        h5f.create_dataset(outname1,data=residual_ts)
        h5f.create_dataset(outname2,data=betas)
    except:
...
def taskGLM(subj, task, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8):
    ...
    print 'Loading task data for subject', subj, 'and task', task
    h5f = h5py.File(outputdir + subj + '_glmOutput_64k_data.h5','r')
