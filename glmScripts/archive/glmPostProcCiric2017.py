...
# GLM modeling for post-processing using Ciric et al. 2017 best-practices
...
# General parameters
datadir = '/projects3/TaskFCMech/data/hcppreprocessedmsmall/'
framesToSkip = 5
...
# directory containing nuisance regressors
nuis_reg_dir = datadir + 'nuisanceRegressors/'
outputdir = '/projects3/TaskFCMech/data/hcppreprocessedmsmall/hcpPostProcCiric/' 
#
...
def main():
    for subj in subjNums:
        for run in allRuns:
            ...
            nuisanceRegressionGlasserParcellation(subj, run, outputdir, model='24pXaCompCorXVolterra',spikeReg=False,zscore=False,nproc=10)


def nuisanceRegressionGlasserParcellation(subj, run, outputdir,  model='24pXaCompCorXVolterra', spikeReg=False, zscore=False, nproc=8):
    """
    ...
    rawfile = datadir + '/hcpPreprocessedData/' + subj + '_GlasserParcellated_' + run + '_LR.csv'
...
def loadStimFiles_task_HCP(subj, gsr=True, volterra=False):
...
#    b0Regressor.shape = (totalTRs,1)
    ...
    # If want to use the 36p (or some variant of the model) include the second order regressor terms
    if volterra:
        noiseRegressors2 =  noiseRegressors**2
        noiseRegressors = np.hstack((noiseRegressors,noiseRegressors2))
